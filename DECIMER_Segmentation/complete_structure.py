...
def get_edge_line(node_1: Tuple[int, int],
...
def get_euklidian_distance(point_1: Tuple[int, int],
...
def set_x_range(x_distance: float,
                eukl_distance: float,
...
def get_next_pixel_to_check(bounding_box: np.array,
                            node_index: int,
                            x_step: int,
    ...
    slope, intercept = get_edge_line(bounding_box[node_index],
...
def adapt_x_values(bounding_box: np.array,
                   node_index: int,
                   image_shape: Tuple[int, int, int]
                   ) -> np.array:
    """
...
def get_contour_seeds(image_array: np.array,
    ...
    for node_index in range(len(bounding_box)):
        ...
        if x_diff == 0:
            bounding_box = adapt_x_values(bounding_box=bounding_box,
                                          node_index=node_index,
                                          image_shape=image_array.shape)
        eukl_dist = get_euklidian_distance(bounding_box[node_index],
                                           bounding_box[node_index-1])
        ...
        for step in x_diff_range:
            x, y = get_next_pixel_to_check(bounding_box,
                                           node_index,
                                           step,
                                           image_array.shape)
...
def get_seeds(image_array: np.array,
              mask_array: np.array
              ) -> List[Tuple[int, int]]:
    """
...
def get_neighbour_pixels(seed_pixel: Tuple[int, int],
                         image_shape: Tuple[int, int, int]
                         ) -> List[Tuple[int, int]]:
    """
...
def expand_masks(image_array: np.array,
                 seed_pixels: List[Tuple[int, int]],
                 mask_array: np.array, contour_expansion=False
                 ) -> np.array:
    """
...
def expansion_coordination(mask_array: np.array, image_array: np.array) -> np.array:
    ...
    if seed_pixels != []:
        ...
        seed_pixels = get_contour_seeds(image_array=image_array, bounding_box=polygon[0])
